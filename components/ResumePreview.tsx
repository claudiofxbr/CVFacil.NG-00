...
import { LegalBlackTemplate } from './templates/LegalBlackTemplate';
import { EngineerNavyTemplate } from './templates/EngineerNavyTemplate';
import { GrungeGrayTemplate } from './templates/GrungeGrayTemplate';
import { FashionDarkRedTemplate } from './templates/FashionDarkRedTemplate';
...
const ResumePreview: React.FC<ResumePreviewProps> = ({ data }) => {
  ...
  // --- Templates estruturais dedicados (componentes próprios) ---
  ...
  if (templateId === 'corporate-blue') return <CorporateBlueTemplate data={data} isDark={isDark} />;
  if (templateId === 'legal-black') return <LegalBlackTemplate data={data} isDark={isDark} />;
  if (templateId === 'engineer-navy') return <EngineerNavyTemplate data={data} isDark={isDark} />;
  if (templateId === 'grunge-gray') return <GrungeGrayTemplate data={data} isDark={isDark} />;
  if (templateId === 'fashion-red') return <FashionDarkRedTemplate data={data} isDark={isDark} />;
        ...
        <div className="max-w-4xl mx-auto space-y-12">
          ...
          <header className="flex flex-col md:flex-row gap-8 items-center md:items-start">
            <div className="w-32 md:w-40 flex-shrink-0 relative">
               ...
               <img src={data.avatarUrl || DEFAULT_AVATAR_PLACEHOLDER} alt={data.fullName} className={`w-full rounded-xl shadow-xl relative z-10 grayscale hover:grayscale-0 transition-all duration-500 ${photoFrameStyle}`} />
            </div>
      ...
      <div className={`w-full flex flex-col md:flex-row font-sans ${containerClass}`}>
        <aside className={`w-full md:w-1/3 p-8 space-y-8 ${sidebarClass}`}>
          <div className="w-32 mx-auto border-4 border-blue-400 mb-6 bg-white">
            <img src={data.avatarUrl || DEFAULT_AVATAR_PLACEHOLDER} alt={data.fullName} className={`w-full ${photoFrameStyle}`} />
          </div>
      ...
      <div className={`w-full p-8 font-display ${bgClass}`}>
         <header className="text-center border-b-4 border-red-600 pb-8 mb-8">
            <div className={`w-24 mx-auto mb-6 p-1 ${headerBg}`}>
                <img src={data.avatarUrl || DEFAULT_AVATAR_PLACEHOLDER} alt="Me" className={`w-full ${photoFrameStyle}`} />
            </div>
      ...
      <div className={`w-full p-8 md:p-12 font-sans ${bgClass}`}>
         <div className="flex flex-col md:flex-row gap-8 mb-12 items-center">
            <div className="w-24 overflow-hidden rounded-lg shadow-lg shadow-emerald-200/20">
                <img src={data.avatarUrl || DEFAULT_AVATAR_PLACEHOLDER} className={`w-full ${photoFrameStyle}`} alt="Profile" />
            </div>
            <div>
              ...
              <aside className={`col-span-4 p-6 rounded-2xl border h-fit ${sidebarBg}`}>
                 <div className="mb-8 text-center">
                    <div className="w-32 mx-auto border-4 border-indigo-500 mb-4 shadow-lg shadow-indigo-500/20 overflow-hidden rounded-lg">
                        <img src={data.avatarUrl || DEFAULT_AVATAR_PLACEHOLDER} className={`w-full ${photoFrameStyle}`} alt="Me" />
                    </div>
                 </div>
                 <div className="mb-8">
        ...
        <div className={`w-full p-12 font-sans ${bgClass}`}>
           <div className={`flex justify-between items-end mb-16 border-l-4 pl-8 ${borderClass}`}>
              ...
              <div className={`w-32 hidden md:block border p-1 ${isDark ? 'border-neutral-700' : 'border-neutral-300'}`}>
                 <img src={data.avatarUrl || DEFAULT_AVATAR_PLACEHOLDER} className={`w-full ${photoFrameStyle} grayscale`} alt="Me" />
              </div>
           </div>
           ...
           <div className={`border p-8 relative ${border}`}>
              <div className={`absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 px-8 ${frameBg}`}>
                 <div className="w-24 rounded-full border-2 border-violet-500 overflow-hidden mx-auto">
                    <img src={data.avatarUrl || DEFAULT_AVATAR_PLACEHOLDER} className={`w-full ${photoFrameStyle}`} alt="Avatar"/>
                 </div>
              </div>
  ...
  // ─── TECH DARK PRO ────────────────────────────────────────────────────────────
  if (templateId === 'tech') {
    const bg = isDark ? 'bg-slate-950 text-slate-200' : 'bg-slate-50 text-slate-800';
    const headerBg = isDark ? 'bg-slate-900' : 'bg-slate-800';
    ...
    return (
      <div className={`w-full font-sans ${bg}`}>
        {/* ── Header Banner ── */}
        <div className={`${headerBg} px-8 md:px-12 py-8 flex items-center gap-8`}>
          <div className="w-24 md:w-28 flex-shrink-0 rounded-xl overflow-hidden border-2 ring-4 ring-emerald-500/20" style={{ borderColor: accent }}>
            <img src={data.avatarUrl || DEFAULT_AVATAR_PLACEHOLDER} alt={data.fullName} className={`w-full ${photoFrameStyle}`} />
          </div>
          <div className="flex-1 min-w-0">
            <h1 className="text-3xl md:text-4xl font-display font-black text-white mb-1">
              {data.fullName.split(' ')[0]} <span style={{ color: accent }}>{data.fullName.split(' ').slice(1).join(' ')}</span>
            </h1>
            <p className="font-mono text-sm mb-3" style={{ color: accent }}>{`> ${data.role}`}</p>
            <p className={`text-xs md:text-sm leading-relaxed ${muted} line-clamp-2`}>{data.summary}</p>
          </div>
          <div className="hidden md:flex flex-col gap-2 text-xs text-right flex-shrink-0" style={{ color: accent }}>
            {data.email && <span className="flex items-center gap-1 justify-end"><span className="material-symbols-outlined text-[14px]">mail</span>{data.email}</span>}
            {data.phone && <span className="flex items-center gap-1 justify-end"><span className="material-symbols-outlined text-[14px]">call</span>{data.phone}</span>}
            {data.linkedin && <span className="flex items-center gap-1 justify-end"><span className="material-symbols-outlined text-[14px]">link</span><span className="truncate max-w-[140px]">{data.linkedin}</span></span>}
            {data.portfolio && <span className="flex items-center gap-1 justify-end"><span className="material-symbols-outlined text-[14px]">language</span>{data.portfolio}</span>}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 md:p-10">
          {/* Sidebar */}
          <aside className={`lg:col-span-3 space-y-5 p-5 rounded-2xl border ${sidebarBg}`}>
            {/* Skills */}
            {data.skills.length > 0 && (
              <section>
                <h3 className="text-[10px] font-mono font-bold uppercase tracking-[0.2em] mb-3" style={{ color: accent }}>// skills</h3>
                <div className="space-y-3">
                  {data.skills.map(s => (
                    <div key={s.id}>
                      ...
                      <div className={`h-1 rounded-full ${isDark ? 'bg-slate-800' : 'bg-slate-100'}`}>
                        <div className="h-full rounded-full transition-all" style={{ width: `${s.level}%`, background: accent }} />
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {data.languages.length > 0 && (
              <section>
                <h3 className="text-[10px] font-mono font-bold uppercase tracking-[0.2em] mb-3" style={{ color: accent }}>// idiomas</h3>
                <div className="space-y-2">
                  {data.languages.map(l => (
                    <div key={l.id} className={`flex justify-between items-center text-xs ${muted}`}>
                      <span className="font-bold text-slate-300">{l.name}</span>
                      <span className="font-mono text-[10px] px-1.5 py-0.5 rounded" style={{ background: `${accent}20`, color: accent }}>{l.level}</span>
                    </div>
                  ))}
                </div>
              </section>
            ...
            {data.hobbies.length > 0 && (
              <section>
                <h3 className="text-[10px] font-mono font-bold uppercase tracking-[0.2em] mb-3" style={{ color: accent }}>// interesses</h3>
                <div className="flex flex-wrap gap-1">
                  {data.hobbies.map((h, i) => (
                    <span key={i} className={`text-[10px] px-2 py-0.5 rounded font-mono ${isDark ? 'bg-slate-800 text-slate-400' : 'bg-slate-100 text-slate-500'}`}>{h}</span>
                  ))}
                </div>
              </section>
            )}
          ...
          <main className="lg:col-span-9 space-y-8">
            ...
            {data.experiences.length > 0 && (
              <section>
                <h2 className="text-[10px] font-mono font-bold uppercase tracking-[0.2em] mb-5" style={{ color: accent }}>// experiência_profissional</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {data.experiences.map(exp => (
                    <div key={exp.id} className={`p-5 rounded-xl border hover:border-emerald-500/40 transition-all ${cardBg}`}>
                      <div className="flex justify-between items-start mb-2 gap-2">
                        <h3 className={`font-bold text-sm ${isDark ? 'text-white' : 'text-slate-900'}`}>{exp.role}</h3>
                        <span className={`text-[10px] font-mono flex-shrink-0 ${muted}`}>{exp.period}</span>
                      </div>
                      <p className="font-bold text-xs mb-2" style={{ color: accent }}>{exp.company}</p>
            ...
            {data.education.length > 0 && (
              <section>
                <h2 className="text-[10px] font-mono font-bold uppercase tracking-[0.2em] mb-5" style={{ color: accent }}>// formação_acadêmica</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {data.education.map(edu => (
                    <div key={edu.id} className={`p-4 rounded-xl border-l-4 ${cardBg}`} style={{ borderLeftColor: accent }}>
                      <p className={`font-bold text-sm mb-1 ${isDark ? 'text-white' : 'text-slate-900'}`}>{edu.degree}</p>
                      <p className={`text-xs ${muted}`}>{edu.institution}</p>
                      <p className="text-[10px] font-mono mt-2" style={{ color: accent }}>{edu.year}</p>
                    </div>
                  ))}
                </div>
              </section>
  ...
  if (templateId === 'timeline') {
    const bg = isDark ? 'bg-zinc-950 text-zinc-200' : 'bg-white text-zinc-800';
    const accent = '#f59e0b'; // amber
    const sectionTitleCls = `text-[11px] font-bold uppercase tracking-[0.25em] mb-5`;
    const muted = isDark ? 'text-zinc-500' : 'text-zinc-400';
    const cardBorder = isDark ? 'border-zinc-800' : 'border-zinc-100';

    return (
      <div className={`w-full font-sans ${bg} pb-16`}>
        {/* Header */}
        <header className={`px-8 md:px-16 pt-12 pb-8 border-b ${isDark ? 'border-zinc-800' : 'border-zinc-100'}`}>
          <div className="flex flex-col md:flex-row items-center md:items-end gap-6">
            <div className="relative">
              <div className="w-20 h-20 rounded-full overflow-hidden" style={{ outline: `3px solid ${accent}`, outlineOffset: '2px' }}>
            ...
            <div className="text-center md:text-left">
              <h1 className={`text-3xl md:text-4xl font-display font-bold mb-1 ${isDark ? 'text-white' : 'text-zinc-900'}`}>
                {data.fullName}
              </h1>
              <p className="font-semibold text-lg" style={{ color: accent }}>{data.role}</p>
            </div>
            <div className={`md:ml-auto hidden md:flex flex-col gap-1 text-xs ${muted} text-right`}>
              {data.email && <span>✉ {data.email}</span>}
              {data.phone && <span>📞 {data.phone}</span>}
              {data.linkedin && <span className="truncate max-w-[200px]">🔗 {data.linkedin}</span>}
            </div>
          </div>
          {data.summary && (
            <p className={`mt-5 text-sm leading-relaxed max-w-3xl ${muted}`}>{data.summary}</p>
          )}
        </header>

        <div className="px-8 md:px-16 mt-10 grid grid-cols-1 lg:grid-cols-3 gap-10">
          {/* Timeline principal */}
          <div className="lg:col-span-2 space-y-10">
            {data.experiences.length > 0 && (
              <section>
                <h2 className={sectionTitleCls} style={{ color: accent }}>Experiência</h2>
                <div className="relative">
                  <div className={`absolute left-3 top-0 bottom-0 w-px ${isDark ? 'bg-zinc-800' : 'bg-zinc-200'}`} />
                  <div className="space-y-8">
                    {data.experiences.map(exp => (
                      <div key={exp.id} className="relative pl-10">
                        <div className="absolute left-0 top-1.5 w-6 h-6 rounded-full flex items-center justify-center" style={{ background: accent }}>
                          <div className="w-2 h-2 rounded-full bg-white" />
                        </div>
                        <div className="flex flex-wrap items-baseline gap-3 mb-1">
                          <h3 className={`font-bold text-base ${isDark ? 'text-white' : 'text-zinc-900'}`}>{exp.role}</h3>
                          <span className={`text-xs font-medium ${muted}`}>{exp.period}</span>
                        </div>
                        <p className="font-bold text-sm mb-2" style={{ color: accent }}>{exp.company}</p>
                        <p className={`text-sm leading-relaxed ${muted}`}>{exp.description}</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {data.education.length > 0 && (
              <section>
                <h2 className={sectionTitleCls} style={{ color: accent }}>Formação</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {data.education.map(edu => (
                    <div key={edu.id} className={`p-4 rounded-xl border ${cardBorder} ${isDark ? 'bg-zinc-900' : 'bg-zinc-50'}`}>
                      <p className={`font-bold text-sm mb-1 ${isDark ? 'text-white' : 'text-zinc-900'}`}>{edu.degree}</p>
                      <p className={`text-xs ${muted}`}>{edu.institution}</p>
                      <span className="inline-block text-[10px] font-bold mt-2 px-2 py-0.5 rounded text-white" style={{ background: accent }}>{edu.year}</span>
                    </div>
                  ))}
                </div>
              </section>
          ...
          <div className="space-y-8">
            {data.skills.length > 0 && (
              <section>
                <h2 className={sectionTitleCls} style={{ color: accent }}>Habilidades</h2>
                <div className="space-y-4">
                  {data.skills.map(s => (
                    <div key={s.id}>
                      <div className="flex justify-between text-xs mb-1">
                        <span className={`font-bold ${isDark ? 'text-zinc-300' : 'text-zinc-700'}`}>{s.name}</span>
                        <span style={{ color: accent }}>{s.level}%</span>
                      </div>
            ...
            {data.languages.length > 0 && (
              <section>
                <h2 className={sectionTitleCls} style={{ color: accent }}>Idiomas</h2>
                <div className="space-y-2">
                  {data.languages.map(l => (
                    <div key={l.id} className={`flex justify-between items-center border-b pb-2 last:border-0 ${cardBorder}`}>
                      <span className={`text-sm font-medium ${isDark ? 'text-zinc-300' : 'text-zinc-700'}`}>{l.name}</span>
                      <span className="text-xs font-bold" style={{ color: accent }}>{l.level}</span>
                    </div>
                  ))}
                </div>
              </section>
            ...
            <section className="md:hidden">
              <h2 className={sectionTitleCls} style={{ color: accent }}>Contato</h2>
            ...
            {data.hobbies.length > 0 && (
              <section>
                <h2 className={sectionTitleCls} style={{ color: accent }}>Interesses</h2>
                <div className="flex flex-wrap gap-2">
                  {data.hobbies.map((h, i) => (
                    <span key={i} className={`text-xs px-2 py-1 rounded border ${isDark ? 'border-zinc-800 text-zinc-400' : 'border-zinc-200 text-zinc-500'}`}>{h}</span>
                  ))}
                </div>
              </section>
            )}
  ...
  if (templateId === 'creative') {
    ...
    const sidebarBg = isDark ? 'bg-violet-950' : 'bg-violet-900';
    const mainBg = isDark ? 'bg-[#0d0d1a] text-slate-200' : 'bg-white text-slate-800';
    const muted = isDark ? 'text-slate-400' : 'text-slate-500';
    const cardBg = isDark ? 'bg-slate-900 border-slate-800' : 'bg-slate-50 border-slate-200';
    const sectionTitle = `text-[10px] font-bold uppercase tracking-[0.2em] mb-4`;

    return (
      <div className={`w-full flex flex-col md:flex-row font-sans min-h-[600px]`}>
        {/* ── Painel lateral colorido ── */}
        <aside className={`${sidebarBg} text-white md:w-72 flex-shrink-0 flex flex-col p-8 gap-8`}>
          {/* Avatar */}
          <div className="flex flex-col items-center text-center">
            <div className="w-28 h-28 rounded-full overflow-hidden ring-4 ring-white/20 mb-4">
              <img src={data.avatarUrl || DEFAULT_AVATAR_PLACEHOLDER} alt={data.fullName} className="w-full h-full object-cover object-top" />
            </div>
            <h1 className="text-2xl font-display font-bold leading-tight">{data.fullName}</h1>
            <p className="text-violet-300 text-sm mt-1 font-medium">{data.role}</p>
          </div>
          ...
          {/* Contato */}
          <section>
            <h2 className={`${sectionTitle} text-violet-400`}>Contato</h2>
            <div className="space-y-2 text-sm text-violet-200">
              {data.email && <p className="flex items-start gap-2 break-all"><span className="material-symbols-outlined text-[16px] text-violet-400 mt-0.5 flex-shrink-0">mail</span>{data.email}</p>}
              {data.phone && <p className="flex items-center gap-2"><span className="material-symbols-outlined text-[16px] text-violet-400">call</span>{data.phone}</p>}
              {data.linkedin && <p className="flex items-start gap-2 break-all"><span className="material-symbols-outlined text-[16px] text-violet-400 mt-0.5 flex-shrink-0">link</span><span className="text-xs">{data.linkedin}</span></p>}
              {data.portfolio && <p className="flex items-center gap-2 break-all"><span className="material-symbols-outlined text-[16px] text-violet-400">language</span>{data.portfolio}</p>}
            </div>
          </section>
          ...
          {data.skills.length > 0 && (
            <section>
              <h2 className={`${sectionTitle} text-violet-400`}>Habilidades</h2>
              <div className="space-y-3">
                {data.skills.map(s => (
                  <div key={s.id}>
                    <div className="flex justify-between text-xs text-violet-200 mb-1">
                      <span>{s.name}</span><span className="text-violet-300">{s.level}%</span>
                    </div>
                    <div className="h-1 rounded-full bg-violet-800">
                      <div className="h-full rounded-full bg-violet-400" style={{ width: `${s.level}%` }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          {data.languages.length > 0 && (
            <section>
              <h2 className={`${sectionTitle} text-violet-400`}>Idiomas</h2>
              <div className="space-y-2">
                {data.languages.map(l => (
                  <div key={l.id} className="flex justify-between text-sm text-violet-200">
                    <span className="font-medium">{l.name}</span>
                    <span className="text-[10px] font-bold bg-violet-800 px-2 py-0.5 rounded text-violet-300">{l.level}</span>
                  </div>
                ))}
              </div>
            </section>
          ...
          {data.hobbies.length > 0 && (
            <section className="mt-auto">
              <h2 className={`${sectionTitle} text-violet-400`}>Interesses</h2>
              <div className="flex flex-wrap gap-1">
                {data.hobbies.map((h, i) => (
                  <span key={i} className="text-[10px] bg-violet-800/50 text-violet-300 px-2 py-0.5 rounded border border-violet-700/50">{h}</span>
                ))}
              </div>
            </section>
          )}
        ...
        {/* ── Conteúdo principal ── */}
        <main className={`flex-1 p-8 md:p-12 space-y-10 ${mainBg}`}>
          {/* Summary */}
          {data.summary && (
            <div className={`p-5 rounded-2xl border-l-4 ${isDark ? 'bg-violet-950/30 border-violet-600 text-slate-300' : 'bg-violet-50 border-violet-400 text-slate-600'} text-sm leading-relaxed italic`}>
              "{data.summary}"
            </div>
          )}
          ...
          {data.experiences.length > 0 && (
            <section>
              <h2 className={`${sectionTitle}`} style={{ color: accent }}>Experiência Profissional</h2>
              <div className="space-y-5">
                {data.experiences.map(exp => (
                  <div key={exp.id} className={`p-5 rounded-xl border hover:border-violet-500/40 transition-all ${cardBg}`}>
                    <div className="flex flex-wrap items-baseline gap-3 mb-1">
                      <h3 className={`font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{exp.role}</h3>
                      <span className={`text-xs ml-auto ${muted}`}>{exp.period}</span>
                    </div>
                    <p className="font-bold text-sm mb-2" style={{ color: accent }}>{exp.company}</p>
                    <p className={`text-sm leading-relaxed ${muted}`}>{exp.description}</p>
                  </div>
          ...
          {data.education.length > 0 && (
            <section>
              <h2 className={`${sectionTitle}`} style={{ color: accent }}>Formação Acadêmica</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {data.education.map(edu => (
                  <div key={edu.id} className={`p-4 rounded-xl border ${cardBg}`}>
                    <div className="flex items-start gap-3">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-white" style={{ background: accent }}>
                        <span className="material-symbols-outlined text-[16px]">school</span>
                      </div>
                      <div>
                        <p className={`font-bold text-sm ${isDark ? 'text-white' : 'text-slate-900'}`}>{edu.degree}</p>
                        <p className={`text-xs ${muted}`}>{edu.institution}</p>
                        <p className="text-[10px] font-bold mt-1" style={{ color: accent }}>{edu.year} · {edu.type}</p>
                      </div>
                    </div>
                  </div>
                ))}
